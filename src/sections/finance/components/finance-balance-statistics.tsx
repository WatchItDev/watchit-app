...
export default function FinanceBalanceStatistics() {
  ...
  useEffect(() => {
    ...
    const sortedFullCategories = sortedKeys.map(
      (key) => groupedData[key]?.label || '',
    );
  ...
  return (
    <>
      <Card>
        ...
        <Box
          data-testid="balance-statistics"
          sx={{ mt: 3, mx: 0, position: 'relative' }}
        >
