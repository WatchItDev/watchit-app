...
import MoviePlayView from '@src/sections/movie/view/movie-play-view.tsx';
...
export default function MovieDetailsView({ id }: Props) {
  ...
  const getPosterHorizontalCid = (): string => data?.metadata?.attachments?.find((el: any) => el.altTag === 'Horizontal Poster')?.image?.raw?.uri
    ...
    <>
      ...
      <Box sx={{ width: '100%', maxWidth: 'calc(100% - 1.5rem)', maxHeight: '100%', position: 'relative' }}>
        <Box sx={{ display: 'flex', flexDirection: 'row', width: '100%', maxWidth: '100%' }}>
          ...
          <MovieDetailMain post={data} handleSubscribe={handleSubscribe} />
        </Box>
      </Box>
    </>
  );
