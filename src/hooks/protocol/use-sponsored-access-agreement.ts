...
export const useSponsoredAccessAgreement =
  (): UseSponsoredAccessAgreementHook => {
    const [data, setData] =
      useState<SponsoredAccessAgreementDetailsReturn | null>(null);
    ...
    const sponsoredAccessAgreement = async (props: SponsoredAccessParams) => {
      const { holder, campaignAddress, policyAddress, parties, payload } =
        props;
      setLoading(true);
      ...
      try {
        ...
        const sponsoredAccessData = encodeFunctionData({
          ...
          args: [holder, campaignAddress, policyAddress, parties, payload],
        });
