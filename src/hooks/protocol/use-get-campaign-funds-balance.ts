...
import {
  HasAccessError,
  UseGetCampaignFundsBalanceHook,
} from '@src/hooks/protocol/types.ts';

export const useGetCampaignFundsBalance =
  (): UseGetCampaignFundsBalanceHook => {
    ...
    const fetchCampaignFundsBalance = useCallback(
      async (campaignAddress: Address): Promise<bigint> => {
        if (!campaignAddress) {
        ...
        try {
          ...
          // @ts-expect-error
          setError({
            message: err?.message || 'Error fetching funds balance.',
          });
          return 0n;
        } finally {
          setLoading(false);
