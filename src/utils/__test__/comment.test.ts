...
describe('[UTILS]: timeAgo', () => {
  ...
  it('displays time in days if less than a month', () => {
    expect(timeAgo(new Date(Date.now() - 5 * 24 * 60 * 60 * 1000))).toBe(
      '5d ago',
    );
  });

  it('displays time in months if less than a year', () => {
    expect(timeAgo(new Date(Date.now() - 2 * 30 * 24 * 60 * 60 * 1000))).toBe(
      '2mo ago',
    );
  });

  it('displays time in years if more than a year', () => {
    expect(timeAgo(new Date(Date.now() - 2 * 365 * 24 * 60 * 60 * 1000))).toBe(
      '2y ago',
    );
  });
