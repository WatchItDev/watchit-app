...
describe('[UTILS]: Errors', () => {
  describe('ERROR_MESSAGES', () => {
    ...
    it('should have the correct message for NOT_LOGGED_IN error', () => {
      expect(ERROR_MESSAGES[ERRORS.NOT_LOGGED_IN]).toBe(
        'Please log in to continue.',
      );
    });

    it('should have the correct message for UNKNOWN_ERROR error', () => {
      expect(ERROR_MESSAGES[ERRORS.UNKNOWN_ERROR]).toBe(
    ...
    it('should contain placeholders in specific error messages', () => {
      expect(
        containsPlaceholders(
          ERROR_MESSAGES[ERRORS.INSUFFICIENT_ALLOWANCE_ERROR],
        ),
      ).toBe(true);
      expect(
        containsPlaceholders(ERROR_MESSAGES[ERRORS.INSUFFICIENT_FUNDS_ERROR]),
      ).toBe(true);
      expect(
        containsPlaceholders(
          ERROR_MESSAGES[ERRORS.SUBSCRIBE_MINIMUN_DAYS_ERROR],
        ),
      ).toBe(true);
      expect(
        containsPlaceholders(
          ERROR_MESSAGES[ERRORS.ASSET_OWNERSHIP_REGISTER_ERROR],
        ),
      ).toBe(true);
    });
    ...
    it('should have the correct placeholder format in messages', () => {
      const messagesWithPlaceholders =
        Object.values(ERROR_MESSAGES).filter(containsPlaceholders);
    ...
    it('should have consistent wallet-related error messages', () => {
      ...
      walletErrors.forEach((error) => {
        expect(ERROR_MESSAGES[error]).toBeDefined();
        expect(ERROR_MESSAGES[error].toLowerCase()).toMatch(
