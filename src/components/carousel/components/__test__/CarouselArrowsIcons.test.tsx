...
import {
  LeftIcon,
  RightIcon,
} from '@src/components/carousel/components/carousel-arrow-icons.tsx';

describe('[COMPONENTS]: LeftIcon component', () => {
  it('to match snapshot', () => {
    const { baseElement } = render(
      <LeftIcon icon="eva:arrow-ios-forward-fill" isRTL={false} />,
    );
  ...
  it('renders correctly with isRTL false', () => {
    ...
    expect(iconElement.getAttribute('data-sx')).toContain(
      '"transform":" scaleX(-1)"',
    );
...
describe('RightIcon component', () => {
  ...
  it('renders correctly with isRTL false', () => {
    const { getByTestId } = render(
      <RightIcon icon="eva:arrow-ios-forward-fill" isRTL={false} />,
    );
